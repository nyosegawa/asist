...
When the working directory is a folder inside a repository, the job runs in the same folder of the worktree. That folder has to be committed to the repository: a job whose working directory is a folder that isn't committed does not start. Neither the commit that gathers a job's changes in the worktree nor the merge runs the repository's commit hooks (pre-commit, prepare-commit-msg, commit-msg, pre-merge-commit and post-commit). The diff you review shows only the job's own changes, even when the job merged your branch in or rebased onto it. A merge goes into the branch checked out in the repository at that moment, and its name is shown above the diff and on the confirmation screen for a merge you ask for in the conversation. Before merging, ASIST checks only that the diff still starts from the same commit, the one where the job's branch and the checked-out branch parted. If that commit has changed, the merge stops and shows the reason with the new diff. A branch created from the same commit, for example with git switch -c, keeps that commit, so the merge goes into that branch. Nothing can be merged while the checked-out branch shares no history with the job. ASIST does not merge a job that touched a submodule: one that changed the commit a submodule points to or .gitmodules, added or removed a submodule, or committed or changed files inside one. Such a job keeps its worktree and waits, with the submodules, the branch and the location of the worktree shown above the diff. Commits made inside a submodule exist only in the copy in that worktree, and git submodule update in your checkout cannot fetch them. Push them from the worktree and merge the branch yourself, or discard the job. Discarding deletes that copy too, so pressing "Discard" on the card also brings up a confirmation screen.
