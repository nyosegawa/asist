...
- 会話から頼んだ Agent のジョブは、始める前と続ける前に確認画面が出て、「ジョブを始める」を押したときだけ動きます。ジョブの変更を取り込むときも、承認してから取り込みます。
...
ASIST は [MIT License](https://github.com/nyosegawa/asist/blob/main/LICENSE) で配布しているソフトウェアで、保証はありません。ASIST を使って起きた損害(データが失われること、かかった料金、誤って送ったメールなど)について、作者と著作権者は責任を負いません。このページは使うときに気をつけることを説明するもので、ライセンスの内容を変えるものではありません。
