...
設定、タスク、タイマー、ジョブなどの JSON のファイルは、形式の版を持ちます。ASIST を新しくして古い版のファイルを開くと、元のファイルを `<名前>.v<版>.json` として同じ場所に残してから、今の形式に移します。今の ASIST より新しい ASIST が書いたファイルや、壊れていて読めないファイルがあるときは、ファイルの場所と理由を表示して止まり、元のファイルには手を付けません。
