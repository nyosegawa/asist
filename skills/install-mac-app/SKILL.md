...
3. Build and install. This takes several minutes, so run it in the background.
   ...
   - The signing identity can be overridden with `CSC_NAME`. Without it, the script uses the Apple Development identity in the keychain and stops when there is none or more than one. In that case check with `security find-identity -v -p codesigning` and ask the user.
